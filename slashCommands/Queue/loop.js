...
module.exports = {
	...
	description: "ปิด/เปิด Loop", //the command description for Slash Command Overview
	cooldown: 5,
				...
				if (newQueue.repeatMode == 0) {
					interaction.reply({
						embeds: [new MessageEmbed()
						  .setColor(ee.color)
						  .setTimestamp()
						  .setTitle(`${client.allEmojis.x} **ปิด LOOP**`)
					...
					interaction.reply({
						embeds: [new MessageEmbed()
						  .setColor(ee.color)
						  .setTimestamp()
						  .setTitle(`🔁 **เปิด __Song__-Loop** ||(ปิด **Queue-Loop**)||`)
					...
					interaction.reply({
						embeds: [new MessageEmbed()
						  .setColor(ee.color)
						  .setTimestamp()
						  .setTitle(`🔂 **เปิด __Queue__-Loop!** ||(ปิด **Song-Loop**)||`)
						  .setFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true}))]
						})
				}
			} catch (e) {
