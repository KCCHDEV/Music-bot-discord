...
module.exports = {
	...
	description: "ลบเพลง", //the command description for Slash Command Overview
	cooldown: 10,
	...
	options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!
		...
		}, //to use in the code: interacton.getInteger("ping_amount")
		{
			"Integer": {
				name: "how_many",
				description: "คุณต้องการลบเพลงออกจากที่นั่นกี่เพลง? (Default: 1)",
				required: false
			}
		}, //to use in the code: interacton.getInteger("ping_amount")
		...
		try {
			...
			try {
				...
				if (songIndex > newQueue.songs.length - 1) return interaction.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **ไม่มีเพลงนี้!**`)
						.setDescription(`**เพลงสุดท้ายในคิวมี : \`${newQueue.songs.length}\`**`)
					],
					ephemeral: true
				})
				if (songIndex <= 0) return interaction.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **คุณไม่สามารถลบเพลงปัจจุบัน (0)!**`)
						.setDescription(`**ใช้ \`${client.settings.get(guild.id, "prefix")}skip\` (Slash)Command**`)
					],
					ephemeral: true
				})
				if (amount <= 0) return interaction.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **คุณต้องลบอย่างน้อย 1 เพลง!**`)
					],
					ephemeral: true
				})
				...
				interaction.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`🗑 **ลบ ${amount} เพลง${amount > 1 ?"s": ""} Queue!**`)
