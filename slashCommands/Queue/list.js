...
module.exports = {
	...
	description: "เพลงทั้งหมด", //the command description for Slash Command Overview
	cooldown: 10,
			...
			try {
				...
				for (let i = 0; i < theSongs.length; i += 10) {
					...
					const embed = new MessageEmbed()
						...
						.setDescription(`${info}`)
					if (i < 10) {
						embed.setTitle(`📑 **Top ${theSongs.length > 50 ? 50 : theSongs.length} | จำนวนเพลงของ ${guild.name}**`)
						embed.setDescription(`**(0) เพลง:**\n> [\`${theSongs[0].name.replace(/\[/igu, "{").replace(/\]/igu, "}")}\`](${theSongs[0].url})\n\n${info}`)
					}
					embeds.push(embed);
					k += 10; //Raise k to 10
				}
				embeds[embeds.length - 1] = embeds[embeds.length - 1]
					.setFooter(ee.footertext + `\n${theSongs.length} เพลงที่รอเล่น | ระยะเวลา: ${newQueue.formattedDuration}`, ee.footericon)
				let pages = []
