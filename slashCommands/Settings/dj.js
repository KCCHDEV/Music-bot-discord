...
module.exports = {
  ...
  description: "ตั้งค่า Dj!", //the command description for helpcmd [OPTIONAL]
  ...
  options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!	
    ...
    }, //here the second array input MUST BE A STRING // TO USE IN THE CODE: interacton.getString("what_ping")
    {
      "Role": {
        name: "which_role",
        description: "คุณต้องการเพิ่ม/ลบบทบาทใด",
        required: true
      }
      ...
      if (add_remove == "add") {
        if (client.settings.get(guild.id, "djroles").includes(Role.id)) {
          return interaction.reply({
            ...
            embeds: [
              ...
              .setTitle(`${client.allEmojis.x} **บทบาทนี้เป็น DJ-ROLE แล้ว!**`)
            ],
          })
        }
        ...
        return interaction.reply({
          ...
          embeds: [
            ...
            .setColor(ee.color)
            .setFooter(ee.footertext, ee.footericon)
            .setTitle(`${client.allEmojis.check_mark} **บทบาท \`${Role.name}\` ได้เพิ่มไปยัง ${client.settings.get(guild.id, "djroles").length - 1} DJ-Roles!**`)
        ...
        if (!client.settings.get(guild.id, "djroles").includes(Role.id)) {
          return interaction.reply({
            ...
            embeds: [
              ...
              .setColor(ee.wrongcolor)
              .setFooter(ee.footertext, ee.footericon)
              .setTitle(`${client.allEmojis.x} **บทบาทนี้ไม่ใช่ DJ-ROLE**`)
            ],
          })
        }
        ...
        return interaction.reply({
          ...
          embeds: [
            ...
            .setColor(ee.color)
            .setFooter(ee.footertext, ee.footericon)
            .setTitle(`${client.allEmojis.check_mark} **บทบาท \`${Role.name}\` ถูกลบออกจาก ${client.settings.get(guild.id, "djroles").length} DJ-Roles!**`)
            .addField(`🎧 **DJ-Role${client.settings.get(guild.id, "djroles").length > 1 ? "s": ""}:**`, `>>> ${djs}`, true)
          ],
        })
      }
