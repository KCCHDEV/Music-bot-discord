...
module.exports = {
  ...
  description: "ส่งข้อความเป็น embed", //the command description for Slash Command Overview
  cooldown: 5,
  ...
  options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!
	//INFORMATIONS! You can add Options, but mind that the NAME MUST BE LOWERCASED! AND NO SPACES!!!, for the CHOCIES you need to add a array of arrays; [ ["",""] , ["",""] ] 
		...
		{"String": { name: "title", description: "หัวข้อ", required: true }}, //to use in the code: interacton.getString("title")
		{"String": { name: "description", description: "ข้อความ [ +n+ = ขึ้นบรรทัดใหม่ ]", required: true }}, //to use in the code: interacton.getString("description")
		{"String": { name: "color", description: "ต้องการสีอะไร", required: false }}, //to use in the code: interacton.getString("color")
		//{"User": { name: "ping_a_user", description: "To Ping a user lol", required: false }}, //to use in the code: interacton.getUser("ping_a_user")
		{"Channel": { name: "in_where", description: "ต้องการให้ส่งไปที่ห้องใหน", required: false }}, //to use in the code: interacton.getChannel("what_channel")
  ...
  run: async (client, interaction) => {
    try{
		...
		await interaction.reply({content: `กำลังส่งข้อความ....`, ephemeral: true}); 
		//SEND THE EMBED!
		await channel.send({embeds: [embed]});
		//Edit the reply
		interaction.editReply({content: `✅ ข้อความถูกส่งไปแล้วจ้าาาา`, ephemeral: true}); 
    } catch (e) {
