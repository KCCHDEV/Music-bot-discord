...
module.exports = {
	...
	description: "ปิด/เปิด Autoplay", //the command description for Slash Command Overview
	cooldown: 5,
			...
			try {
				...
				interaction.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`**${newQueue.autoplay ? `${client.allEmojis.check_mark} เปิด` :`${client.allEmojis.x} ปิด`} Autoplay!**`)
					  .setFooter(`💢 ใช้งายโดย: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true}))]
				})
			} catch (e) {
