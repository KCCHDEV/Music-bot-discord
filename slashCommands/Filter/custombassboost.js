...
module.exports = {
	...
	description: "เลือก Bassboost แบบกำหนดเอง", //the command description for Slash Command Overview
	cooldown: 5,
	...
	options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!
		...
		//{"Channel": { name: "in_where", description: "In What Channel should I send it?", required: false }}, //to use in the code: interacton.getChannel("what_channel")
		{
			"Integer": {
				name: "bass_gain",
				description: "ต้องการระดับ BASS เท่าไร",
				required: true
			}
			...
			try {
				...
				if (bass_gain > 20 || bass_gain < 0) {
					return interaction.reply({
						embeds: [
							...
							.setColor(ee.wrongcolor)
							.setFooter(ee.footertext, ee.footericon)
							.setTitle(`${client.allEmojis.x} **ระดับ bass ทุกตั้งว่า ต่ำสุด 0 สูงสุด 20**`)
						],
					})
				}
				...
				interaction.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`♨️ **ตั้งค่า Bassboost เป็น ${bass_gain}!**`)
