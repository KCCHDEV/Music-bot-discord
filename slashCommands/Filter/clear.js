...
module.exports = {
	...
	description: "ลบ Filter ทั้งหมด", //the command description for Slash Command Overview
	cooldown: 5,
		...
		try {
			...
			try {
				...
				if (!newQueue || !newQueue.songs || newQueue.songs.length == 0) return interaction.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **นูยังไม่ได้ร้องเพลงอะไรเลยนะ**`)
					],
					ephemeral: true
				})
				...
				interaction.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`🗑 **ลบ Filter ทั้งหมดแล้ว**`)
